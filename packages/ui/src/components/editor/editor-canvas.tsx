...
export default function LyricsEditor({
	...
	const [generatingCellId, setGeneratingCellId] = useState<number | null>(null)
	...
	const updateCellTime = (id: number, timeStart?: number, timeEnd?: number) => {
		setCells((prevCells) =>
			prevCells.map((cell) =>
				cell.id === id ? { ...cell, timeStart, timeEnd } : cell
			)
		)
	}
	...
	const handleAnalyzedVersesUpdate = (result: any) => {
		...
		const lyricCellIds = cells
			.filter((cell) => cell.type === "lyric")
			.map((cell) => cell.id)

		// Now map verseTimes to lyricCellIds 1:1
		verseTimes.forEach(
			(
				verse: { start_time: number | undefined; end_time: number | undefined },
				idx: number
			) => {
	...
	return (
		<div className={`flex flex-1 overflow-hidden ${className}`}>
			...
			<div
				...
				<button
					className="px-2"
					onClick={() => addCell("lyric")}>
					Add Lyrics
				</button>
				<button
					className="px-2"
					onClick={() => addCell("note")}>
					Add Notes
				</button>
				...
				{cells.map((cell, index) => (
					<div
						...
						<Cell
							...
							isGenerating={generatingCellId === cell.id}
						/>
						{/* { cell.type === 'lyric' ? (
                <Cell
                  key={cell.id}
						...
						{selectedCellId === cell.id && (
							<div className="flex justify-between">
								<div>
									...
									<button
										className="px-2 py-1 text-white rounded hover:bg-[#64ffda] hover:text-black"
										onClick={() => insertCell(index, "lyric")}>
										Insert Lyrics
									</button>
									<button
										className="px-2 py-1 text-white rounded hover:bg-[#64ffda] hover:text-black"
										onClick={() => insertCell(index, "note")}>
										Insert Notes
									</button>
									<button
										className="px-2 py-1 text-white rounded hover:bg-[#64ffda] hover:text-black"
								...
								{cell.type === "lyric" && (
									<LyricGenerator
										...
										onGeneratingChange={(isGenerating: boolean) => {
											setGeneratingCellId(isGenerating ? cell.id : null)
										}}
									/>
								)}
							</div>
